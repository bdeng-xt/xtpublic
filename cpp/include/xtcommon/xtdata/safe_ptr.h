...
namespace sf {
	...
	template<typename T, typename mutex_t = std::recursive_mutex, typename x_lock_t = std::unique_lock<mutex_t>,
		...
		class safe_ptr {
		protected:
			std::shared_ptr<T> ptr; //const std::shared_ptr<T> ptr;   // std::experimental::propagate_const<std::shared_ptr<T>> ptr;  // C++17
...
//			template<typename... mutex_type> friend class std::lock_guard;  //template<class... mutex_types> friend class std::lock_guard;  // MSVS2015
//#else
...
//#endif
...
#endif
			template<typename mutex_type> friend class boost::lock_guard;
			template<typename mutex_type> friend class boost::shared_lock;
		...
		public:
			...
			safe_ptr(T* p) : ptr(p), mtx_ptr(std::make_shared<mutex_t>()) {}

			safe_ptr( std::shared_ptr<T> p) : ptr(p), mtx_ptr(std::make_shared<mutex_t>()) {}

			safe_ptr(safe_ptr const & r): ptr(r.ptr), mtx_ptr(std::make_shared<mutex_t>()) {}

			safe_ptr & operator=(const safe_ptr & r ) 
			{
				ptr = r.ptr;
			...
			safe_ptr & operator=(safe_ptr && r)
			{
				(static_cast< std::shared_ptr< T > && >(r.ptr)).swap(ptr);
				mtx_ptr = (std::make_shared<mutex_t>());
				return *this;
			}
	...
	template<typename T, typename mutex_t = std::recursive_mutex, typename x_lock_t = std::unique_lock<mutex_t>,
		...
		public:
			...
			safe_obj(safe_obj const& safe_obj) { boost::lock_guard<mutex_t> lock(safe_obj.mtx); obj = safe_obj.obj; } //safe_obj(safe_obj const& safe_obj) { std::lock_guard<mutex_t> lock(safe_obj.mtx); obj = safe_obj.obj; }
	...
	struct link_safe_ptrs {
		...
		link_safe_ptrs(T1 &first_ptr, Args&... args) {
			boost::lock_guard<T1> lock(first_ptr);  //std::lock_guard<T1> lock(first_ptr);
			typedef typename T1::mtx_t mutex_t;
			std::shared_ptr<mutex_t> old_mtxs[] = { args.mtx_ptr ... }; // to unlock before mutexes will be destroyed
			std::shared_ptr<boost::lock_guard<mutex_t>> locks[] = { std::make_shared<boost::lock_guard<mutex_t>>(*args.mtx_ptr) ... };  //std::shared_ptr<std::lock_guard<mutex_t>> locks[] = { std::make_shared<std::lock_guard<mutex_t>>(*args.mtx_ptr) ... };
	...
	template<typename T> 
	struct contfree_safe_ptr
		: public safe_ptr<T, contention_free_shared_mutex<>, std::unique_lock<contention_free_shared_mutex<>>, shared_lock_guard<contention_free_shared_mutex<>> >
	{
	public:

		contfree_safe_ptr() :
			safe_ptr<T, contention_free_shared_mutex<>, std::unique_lock<contention_free_shared_mutex<>>, shared_lock_guard<contention_free_shared_mutex<>> >(new T())
		{
		}

		contfree_safe_ptr(T* p) :
			safe_ptr<T, contention_free_shared_mutex<>, std::unique_lock<contention_free_shared_mutex<>>, shared_lock_guard<contention_free_shared_mutex<>> >(p)
		{
		} //safe_ptr(T* p) : ptr(p), mtx_ptr(std::make_shared<mutex_t>()) {}

//		safe_ptr(std::shared_ptr<T> p) : ptr(p), mtx_ptr(std::make_shared<mutex_t>()) {}

		contfree_safe_ptr(contfree_safe_ptr const & r) //:
			//ptr(r.ptr), mtx_ptr(std::make_shared<contention_free_shared_mutex<> >())
		{
			this->ptr = r.ptr;
			this->mtx_ptr = std::make_shared<contention_free_shared_mutex<> >();
		}//		safe_ptr(safe_ptr const & r) : ptr(r.ptr), mtx_ptr(std::make_shared<mutex_t>()) {}

		contfree_safe_ptr& operator=(const contfree_safe_ptr& r)
		{
			this->ptr = r.ptr;
			this->mtx_ptr = std::make_shared<contention_free_shared_mutex<> >();
			return *this;
		...
		contfree_safe_ptr(contfree_safe_ptr && r)
		{
			this->ptr.swap(r.ptr);
			this->mtx_ptr= (std::make_shared<contention_free_shared_mutex<> >());
		}
		...
		contfree_safe_ptr& operator=(contfree_safe_ptr && r)
		{
			(static_cast< std::shared_ptr< T > && >(r.ptr)).swap(this->ptr);
			this->mtx_ptr = (std::make_shared<contention_free_shared_mutex<> >());
			return *this;
		}
		//safe_ptr & operator=(safe_ptr && r)
		...
		bool operator!() const { return !(this->ptr); }

		void swap(contfree_safe_ptr & other) noexcept //void swap(safe_ptr & other) noexcept
		{
			this->ptr.swap(other.ptr);
			this->mtx_ptr.swap(other.mtx_ptr);
		}
