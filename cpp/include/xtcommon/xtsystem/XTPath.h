...
*
* \brief  Provide a path class .
*
* \description
*	Designed for a path class.
*/
	...
	*/
	...
	public:
		...
		*/
		...
		XTUtcTimestamp created() const;
		//! Get the path modified UTC timestamp
		XTUtcTimestamp modified() const;
		...
		*/
		static void SetCreated(const Path& path, const XTUtcTimestamp& timestamp);
		//! Set modified UTC timestamp for the given path
		/*!
			\param path - Path to modify
			\param timestamp - Modified timestamp
		*/
		static void SetModified(const Path& path, const XTUtcTimestamp& timestamp);
		...
		*/
		...
		{
			SetModified(path, XTUtcTimestamp());
		}
...
ENUM_FLAGS(XT::FileAttributes)
ENUM_FLAGS(XT::FilePermissions)
