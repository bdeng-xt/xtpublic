#pragma once
#ifndef XT_CONDITION_VARIABLE_H
#define XT_CONDITION_VARIABLE_H

/**
* \file XTConditionVariable.h
*
* \author Bin Deng (bdeng@xtal-tech.com)
*
* \brief  Provide a condition variable class .
*
* \description
*	Designed for a condition variable class.
*/
...
namespace XT
{
	...
	*/
	class XT_COMMON_API ConditionVariable
	{
	public:
		ConditionVariable();
		...
		*/
		bool TryWaitFor(CriticalSection& cs, const XTTimespan& timespan);
		...
		*/
		template <typename TPredicate>
		bool TryWaitFor(CriticalSection& cs, const XTTimespan& timespan, TPredicate predicate);
		...
		/*!
			...
			\return 'true' if the condition variable was successfully notified, 'false' if the timeout was occurred
		*/
		bool TryWaitUntil(CriticalSection& cs, const XTUtcTimestamp& timestamp)
		{
			return TryWaitFor(cs, timestamp - XTUtcTimestamp());
		}
		...
		*/
		template <typename TPredicate>
		bool TryWaitUntil(CriticalSection& cs, const XTUtcTimestamp& timestamp, TPredicate predicate)
		{
			return TryWaitFor(cs, timestamp - XTUtcTimestamp(), predicate);
		}
...
////
	...
	template <typename TPredicate>
	bool ConditionVariable::TryWaitFor(CriticalSection& cs, const XTTimespan& timespan, TPredicate predicate)
	{
		XTTimestamp timeout = XTUtcTimestamp() + timespan;
		while (!predicate())
			if (!TryWaitFor(cs, timeout - XTUtcTimestamp()))
				return predicate();
		return true;
	}
