...
#include <boost/algorithm/string.hpp>// #for boost::iequals , replace strcasecmp
...
namespace XT
{
	...
	typedef void(XTHappyHttp::*ResponseBegin_CB)(const Response* r, void* userdata);
	typedef void(XTHappyHttp::*ResponseData_CB)(const Response* r, void* userdata, const unsigned char* data, int numbytes);
	typedef void(XTHappyHttp::*ResponseComplete_CB)(const Response* r, void* userdata);
...
//add by zhuwx 2015-10-12--封装一个简易使用的happyhttp类
	class XTHappyHttp
	{
	private:
		int m_iCount;                         //此次请求返回报文体字节数
		bool m_bIsComplete;                   //是否完成此次请求
		int m_iStatus;                        //此次请求返回的http状态码
		std::string m_strReason;                   //此次请求返回的http状态描述
		std::string m_strRespData;                 //此次请求返回的报文体

	public:
		XTHappyHttp() : m_iCount(0), m_bIsComplete(false), m_iStatus(-1), m_strReason(""), m_strRespData("") {

		}

		/**post一个http请求.
		* strUrl:形如"http://10.182.20.38:9808/open/workflow/realNameInter".
		* body:请求参数,形如"user=root&pass=123456".
		* strRespData:返回报文数据.
		* strErr:如果函数返回值小于0,这里包含错误消息.
		*/
		int postHttpRequest(const std::string& strUrl, const char* body, std::string& strRespData, std::string& strErr);

		///**转换字符串的编码
		//* strSrc:源字符串
		//* strSrcCharset:源字符串编码,如GBK
		//* strDest:目标字符串
		//* strDestCharset:目标字符串编码,如UTF8
		//* strErr:如果函数返回值小于0,这里包含错误消息.
		//*/
		//int myConvertCharset(
		//	const string& strSrc,
		//	const string& strSrcCharset,
		//	string& strDest,
		//	const string& strDestCharset,
		//	string& strErr);

		std::string urlEncoding(const std::string &sIn);

	private:
		void onBegin(const Response* r, void* userdata);
		void onData(const Response* r, void* userdata, const unsigned char* data, int n);
		void onComplete(const Response* r, void* userdata);
