								   ...
								   const std::string& genericTicks, bool snapshot, const TagValueListSPtr& mktDataOptions)
{
	...
	if( m_serverVersion >= MIN_SERVER_VER_LINKING) {
		...
		const int mktDataOptionsCount = mktDataOptions.get() ? (int)(mktDataOptions->size()) : 0;
...
void EClient::reqMktDepth( TickerId tickerId, const Contract& contract, int numRows, const TagValueListSPtr& mktDepthOptions)
{
	...
	if( m_serverVersion >= MIN_SERVER_VER_LINKING) {
		...
		const int mktDepthOptionsCount = mktDepthOptions.get() ? (int)(mktDepthOptions->size()) : 0;
										  ...
										  int useRTH, int formatDate, const TagValueListSPtr& chartOptions)
{
	...
	if( m_serverVersion >= MIN_SERVER_VER_LINKING) {
		...
		const int chartOptionsCount = chartOptions.get() ? (int)(chartOptions->size()) : 0;
										...
										const TagValueListSPtr& realTimeBarsOptions)
{
	...
	if( m_serverVersion >= MIN_SERVER_VER_LINKING) {
		...
		const int realTimeBarsOptionsCount = realTimeBarsOptions.get() ? (int)(realTimeBarsOptions->size()) : 0;
											   ...
											   const ScannerSubscription& subscription, const TagValueListSPtr& scannerSubscriptionOptions)
{
	...
	if( m_serverVersion >= MIN_SERVER_VER_LINKING) {
		...
		const int scannerSubscriptionOptionsCount = scannerSubscriptionOptions.get() ? (int)(scannerSubscriptionOptions->size()) : 0;
...
void EClient::placeOrder( OrderId id, const Contract& contract, const Order& order)
{
	...
	if( m_serverVersion >= MIN_SERVER_VER_LINKING) {
		...
		const int orderMiscOptionsCount = orderMiscOptions ? (int)(orderMiscOptions->size()) : 0;
